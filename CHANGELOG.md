...
### New
- 新增表情库管理 WebUI（插件页 `meme-manage`）：按贴纸名分类展示表情，每个名字下展示全部变体。
- 网页内支持：新增表情（本地图片上传）、删除表情、重命名表情、删除单个变体，破坏性操作带确认弹窗。
- 新增后端 `web_api` 模块：列表 / 图片预览 / 上传 / 删除 / 重命名 / 变体删除六个接口，兼容新版 `astrbot.api.web` 与旧版 quart 回退；图片预览经路径穿越校验后直接返回文件。

## 2.1.0 - 2026-08-05

### New
- GIF 表情入库不再转 WebP，直接保留原文件，动图效果不丢失。
- GIF 超过 5MB 拒绝入库，返回「图片过大，无法作为表情」。
- 入库命名跟随源文件后缀：GIF 存为 `.gif`，其他格式仍转 `.webp`；同名根文件检测扩展到任意支持后缀，避免静态/动图混合时索引互相覆盖。
